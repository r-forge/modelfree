...
if length( h ) == 1
    ...
    if nargout == 3,
        ...
        if (Lxfit<=split),
            ...
            % small x
            [pfit,etafit,H] = feval(fun_estim,xfit,r,m,x,h,link,guessing,lapsing,...
                K,p,ker,maxiter,tol);
        else
            % large x
        ...
        if (Lxfit<=split),
            ...
            % small x
            [pfit,etafit] = feval(fun_estim,xfit,r,m,x,h,link,guessing,lapsing,...
                K,p,ker,maxiter,tol);
        else
            % large x
            % number of parts into which the fitting is divided
            ...
            for i = 0:(fLx-1),
                % part of the fit
                [pfit1,etafit1] = feval(fun_estim,xfit(i*split + (1:split)),r,m,x,h,...
            ...
            %final part of the fit
            if ((split*fLx)<Lxfit),
                [pfit1,etafit1] = feval(fun_estim,xfit((1+split*fLx):Lxfit),r,m,x,h,...
                    link,guessing,lapsing,K,p,ker,maxiter,tol);
    ...
    if nargout == 3,
        ...
        if (Lxfit<=split),
            ...
            H = [];   
                ...
                for i = 0:(fLx-1),
                 % part of the fit
                    [pfit1,etafit1,H1] = feval(fun_estim,xfit(i*split + (1:split)),r,m,x,...
                ...
                %final part of the fit
                if ((split*fLx)<Lxfit),
                    [pfit1,etafit1,H1] = feval(fun_estim,xfit((1+split*fLx):Lxfit),r,m,x,...
        ...
        if (Lxfit<=split),
            ...
            for i = 0:(fLx-1),
                % part of the fit
                [pfit1,etafit1] = feval(fun_estim,xfit(i*split + (1:split)),r,m,x,h(i*split + (1:split)),...
                    link,guessing,lapsing,K,p,ker,maxiter,tol);
                % put the fits together
                pfit = [pfit;pfit1];
                etafit = [etafit;etafit1];
            ...
            %final part of the fit
            if ((split*fLx)<Lxfit),
                [pfit1,etafit1] = feval(fun_estim,xfit((1+split*fLx):Lxfit),r,m,x,h((1+split*fLx):Lxfit),...
                    link,guessing,lapsing,K,p,ker,maxiter,tol);
