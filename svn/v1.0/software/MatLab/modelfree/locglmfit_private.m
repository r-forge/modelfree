function [pfit,etafit,H] = locglmfit_private(xfit,r,m,x,h,link,guessing,...
...
% returns the hat matrix H. This function is used for small data sets, i.e.
% fewer than 15 observations.
%
% INPUT
%
...
W = diag(repmat(1 ./(eta_mu.*sqrt(mu0 .* (1 - mu0)./m)),nx,1));
...
DetXX = det( X'*X);
    ...
    if( abs(DetXX) < 1e-14),
    	error('Determinant close to 0: bandwidth is too small')
    end
