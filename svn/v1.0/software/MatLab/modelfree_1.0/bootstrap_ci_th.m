function [ci,th0] = bootstrap_ci_th(TH,r,m,x,N,h0,alpha,X,link,guessing,lapsing,...
                        K,p,ker,maxiter,tol)
%
% Bootstrap estimate of a confidence interval at a significance level alpha for 
% the estimated threshold for the local polynomial estimate of the psychometric 
% function with guessing and lapsing rates. The confidence interval is based on 
...
%       threshold estimation are to be obtained; if not given, 1000 equally spaced
...
if( min(x) > min(X) || max(x) < max(X) ) 
        error('Supplied values of X are outside the range of stimulus levels x');
end

checkinput( 'bandwidth', h0 );
if length(alpha)>1,
    error('Significance level must be scalar')
...
if N < 1000,
    warning('modelfree:smallN', 'number of bootstrap samples should be greater than 1000 \n otherwise the results might be unreliable' );
end
...
warning(s_msg_id)
ci(1) = prctile( th_boot, 100 * ( alpha / 2 ) );
ci(2) = prctile( th_boot, 100 * ( 1 - alpha / 2 ) );